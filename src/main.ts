...
const SNAKE_COLORS: [number, number][] = [
    [0x4a7c59, 0xb8d4c8], [0x7a4e2d, 0xd4a882], [0x5c4a7f, 0xc2b0e8], [0x8b5e3c, 0xe0c9b4],
    [0x3a6b6e, 0xa8cdd0], [0x6b4e71, 0xd4b8d9], [0x4e6741, 0xb5c9a8], [0x7a3b3b, 0xd9a8a8],
];
...
class Player {
    ...
    constructor(id: number, name: string, color: number, s: THREE.Scene, startPos: THREE.Vector3) {
...
for (let i = 0; i < TOTAL_TILES; i++) {
    ...
    const tp = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({ map: new THREE.CanvasTexture(tc), transparent: true, opacity: 0.8 })
    );
...
// ── Snakes & Ladders ──────────────────────────────────────────────────────────
interface LadderInfo { group: THREE.Group; axis: THREE.Vector3; midPoint: THREE.Vector3; }
const ladders: LadderInfo[] = [];
const snakePaths: Record<number, THREE.CatmullRomCurve3> = {};
const snakeHeads: { mesh: THREE.Group; baseRotation: THREE.Euler; baseY: number }[] = [];
const snakeBodySegs: { mesh: THREE.Mesh; basePos: THREE.Vector3; side: THREE.Vector3; t: number; length: number }[] = [];

const ladderMat   = new THREE.MeshStandardMaterial({ color: 0xd97706, roughness: 0.8, metalness: 0.1 });
...
function createTextSprite(text: string, clr: string): THREE.Sprite {
    ...
    ctx.fillStyle = clr; ctx.font = 'bold 36px sans-serif';
...
for (const [startStr, endStr] of Object.entries(JUMPS)) {
    ...
    if (start < end) {
        /* ── LADDER ── */
        const bot = startPos.clone(); bot.y += 0.3;
        const top = endPos.clone();   top.y += 0.3;
        ...
        const lRail = new THREE.Mesh(railGeo, ladderMat); lRail.position.x = -0.4;
        const rRail = new THREE.Mesh(railGeo, ladderMat); rRail.position.x =  0.4;
        lg.add(lRail, rRail);
        ...
        lSpr.position.copy(startPos); lSpr.position.y += 1.5;
        boardGroup.add(lSpr);

    } else {
        /* ── SNAKE ── */
        const [sc, bc] = SNAKE_COLORS[snakeColorIdx++ % SNAKE_COLORS.length];
        const skinMat  = new THREE.MeshStandardMaterial({ color: sc, roughness: 0.5 });
        const bellyMat = new THREE.MeshStandardMaterial({ color: bc, roughness: 0.5 });

        const diff = new THREE.Vector3().subVectors(endPos, startPos);
        const len  = diff.length();
        diff.normalize();
        const side = new THREE.Vector3().crossVectors(diff, new THREE.Vector3(0, 1, 0)).normalize();
        ...
        for (let i = 0; i <= numSegs; i++) {
            ...
            seg.position.copy(pt); seg.castShadow = true;
            sg.add(seg);
            snakeBodySegs.push({ mesh: seg, basePos: pt.clone(), side, t, length: len });
        }
        ...
        snakeHeads.push({ mesh: hg, baseRotation: hg.rotation.clone(), baseY: headPt.y });

        const hMesh = new THREE.Mesh(new THREE.SphereGeometry(0.6, 16, 16), skinMat);
        hMesh.scale.set(1, 0.7, 1.2); hg.add(hMesh);

        const snout = new THREE.Mesh(new THREE.SphereGeometry(0.4, 16, 16), bellyMat);
        snout.position.set(0, -0.2, 0.3); snout.scale.set(1.2, 0.5, 1); hg.add(snout);

        const eGeo = new THREE.SphereGeometry(0.15, 8, 8);
        const pGeo = new THREE.SphereGeometry(0.08, 8, 8);
        const lEye = new THREE.Mesh(eGeo, eyeWhiteMat); lEye.position.set(-0.3, 0.3, 0.3); hg.add(lEye);
        const lPup = new THREE.Mesh(pGeo, eyeBlackMat); lPup.position.set(-0.3, 0.35, 0.42); hg.add(lPup);
        const rEye = new THREE.Mesh(eGeo, eyeWhiteMat); rEye.position.set(0.3, 0.3, 0.3); hg.add(rEye);
        const rPup = new THREE.Mesh(pGeo, eyeBlackMat); rPup.position.set(0.3, 0.35, 0.42); hg.add(rPup);

        const tongue = new THREE.Mesh(new THREE.PlaneGeometry(0.2, 0.6), tongueMat);
        tongue.position.set(0, -0.1, 0.8); tongue.rotation.x = Math.PI / 2; hg.add(tongue);

        sg.add(hg); boardGroup.add(sg);

        const sSpr = createTextSprite(`Snake to ${end}`, '#fca5a5');
        sSpr.position.copy(startPos); sSpr.position.y += 2.0;
        boardGroup.add(sSpr);
    }
}
...
function createDiceFaceTexture(num: number): THREE.CanvasTexture {
    ...
    ctx.fill();
    ctx.strokeStyle = '#cdc8c0'; ctx.lineWidth = 4; ctx.stroke();
...
// ── Dice Scene ────────────────────────────────────────────────────────────────
const diceCanvasEl = document.getElementById('dice-canvas') as HTMLCanvasElement;
const diceRenderer = new THREE.WebGLRenderer({ canvas: diceCanvasEl, antialias: true, alpha: true });
diceRenderer.setSize(220, 220);
...
diceFloor.position.y = -1.075; diceFloor.receiveShadow = true;
...
const diceMats = DICE_FACE_VALUES.map(v =>
    new THREE.MeshStandardMaterial({ map: createDiceFaceTexture(v), roughness: 0.22, metalness: 0.03 })
);
...
const DICE_GRAVITY    = -22;
const FLOOR_CONTACT_Y =  0;   // dice center Y when bottom face rests on floor
const DICE_RESTITUTION = 0.42;

let diceIdleTime  = 0;
let isHoldingDice = false;
let lastMouseX    = 0, lastMouseY    = 0;
let throwVX       = 0, throwVZ       = 0;
let throwAngX     = 0, throwAngY     = 0;

function updateDicePhysics(dt: number) {
    dt = Math.min(dt, 0.05); // cap to avoid tunneling on tab switch
    ...
    if (diceMesh.position.y < FLOOR_CONTACT_Y) {
        ...
        if (Math.abs(diceVelocity.y) > 0.5) {
    ...
    if (spin > 0.001) {
        const q = new THREE.Quaternion().setFromAxisAngle(
            diceAngularVelocity.clone().normalize(), spin * dt
        );
        diceMesh.quaternion.premultiply(q);
    }

    // Damping
    diceVelocity.multiplyScalar(0.991);
    diceAngularVelocity.multiplyScalar(0.963);

    // Settle check
    const grounded = diceMesh.position.y <= FLOOR_CONTACT_Y + 0.05;
    if (grounded && diceVelocity.length() < 0.09 && spin < 0.09) {
        snapDiceToFace();
    }
}

function snapDiceToFace() {
    // Find which face is currently most upward
    const up = new THREE.Vector3(0, 1, 0);
    let maxDot = -Infinity, topIdx = 2;
    FACE_NORMALS.forEach((n, i) => {
        const dot = n.clone().applyQuaternion(diceMesh.quaternion).dot(up);
        if (dot > maxDot) { maxDot = dot; topIdx = i; }
    });

    // Rotate so that face is exactly pointing up
    const faceWorldNormal = FACE_NORMALS[topIdx].clone().applyQuaternion(diceMesh.quaternion);
    diceMesh.quaternion.premultiply(
        new THREE.Quaternion().setFromUnitVectors(faceWorldNormal, up)
    );
    diceMesh.position.set(0, FLOOR_CONTACT_Y, 0);
    diceVelocity.set(0, 0, 0);
    diceAngularVelocity.set(0, 0, 0);

    diceLandGlow.intensity = 3.5; // trigger landing flash
    diceState = 'done';
...
function startHold(clientX: number, clientY: number) {
    ...
    diceState = 'held'; isHoldingDice = true;
...
function moveDice(clientX: number, clientY: number) {
    ...
    const dx = clientX - lastMouseX, dy = clientY - lastMouseY;
    // Rotate the die in your hand
    diceMesh.rotation.y += dx * 0.013;
    diceMesh.rotation.x += dy * 0.013;
    // Accumulate throw velocity with exponential smoothing
    throwVX   = throwVX   * 0.65 + dx * 0.038;
    throwVZ   = throwVZ   * 0.65 + dy * 0.038;
    throwAngX = throwAngX * 0.65 + dy * 0.32;
    throwAngY = throwAngY * 0.65 + dx * 0.32;
    lastMouseX = clientX; lastMouseY = clientY;
}
...
function releaseDice() {
    ...
    diceVelocity.set(throwVX * 2.5, 7.5 + spd * 2.5, throwVZ * 2.5);
    diceAngularVelocity.set(throwAngX * 4, throwAngY * 4, (throwAngX - throwAngY) * 2);
}
...
// Touch events
diceCanvasEl.addEventListener('touchstart', (e) => {
    e.preventDefault();
...
window.addEventListener('touchmove', (e) => {
    if (!isHoldingDice) return;
    e.preventDefault();
    moveDice(e.touches[0].clientX, e.touches[0].clientY);
}, { passive: false });

window.addEventListener('touchend', () => releaseDice());
...
function renderSetupInputs() {
    ...
    playerInputsEl.querySelectorAll<HTMLButtonElement>('.remove-player-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            if (pendingCount > 2) { pendingCount--; renderSetupInputs(); }
        });
    });
...
addPlayerBtn.addEventListener('click', () => {
    if (pendingCount < 4) { pendingCount++; renderSetupInputs(); }
});

startGameBtn.addEventListener('click', () => {
    const inputs = playerInputsEl.querySelectorAll<HTMLInputElement>('input[type="text"]');
    const names: string[] = [];
    inputs.forEach((inp, i) => names.push(inp.value.trim() || `Player ${i + 1}`));

    players = names.map((name, i) =>
        new Player(i, name, PLAYER_COLORS[i], scene, tilePositions[1])
    );
...
function animate() {
    ...
    snakeHeads.forEach((sh, idx) => {
        ...
        sh.mesh.rotation.x = sh.baseRotation.x + Math.sin(time * 0.5 + idx) * 0.10;
        ...
        if (player?.isMoving) {
            ...
            if (player.currentJump) {
                ...
                if (player.jumpProgress >= 1.0) {
                    ...
                    if (curve) player.mesh.position.copy(off(curve.getPoint(player.jumpProgress)));
                } else {
                    // climb: lerp from start tile to target (top of ladder)
                    const sp = off(tilePositions[player.currentTile]);
                    player.mesh.position.lerpVectors(sp, player.targetPosition, player.jumpProgress);
                }
                ...
                if (JUMPS[player.currentTile] && nextNum === JUMPS[player.currentTile]) {
                    ...
                    if (player.hopProgress >= 1.0) {
                        ...
                        player.mesh.position.lerpVectors(
                            player.startHopPosition, player.targetPosition, player.hopProgress
                        );
